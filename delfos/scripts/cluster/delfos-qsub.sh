	...
	then
		...
		do
			...
			do
				if [ $((q%3)) == 0 ]
				then
					qsub -q queue1 -v experimentFolder=${i} "./delfos-qsub-job.sh";echo "submited to queue1"
				else
					qsub -q queue2 -v experimentFolder=${i} "./delfos-qsub-job.sh";echo "submited to queue2"
				fi
				let k++
				let q++
			done
