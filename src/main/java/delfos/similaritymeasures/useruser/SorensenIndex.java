 ...
 * @author jcastro
 */
public class SorensenIndex extends SimilarityMeasureAdapter implements CollaborativeSimilarityMeasure, UserUserSimilarity {
    ...
    public double similarity(Collection<CommonRating> commonRatings, RatingsDataset<? extends Rating> ratings) throws CouldNotComputeSimilarity {
        ...
        double ret = commonRatings.stream().findAny().map(commonRating -> {
            ...
            double similarity = 2 * intersectionSize / (user1size + user2size);
