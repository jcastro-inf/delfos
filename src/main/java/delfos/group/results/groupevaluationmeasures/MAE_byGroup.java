 ...
 */
public class MAE_byGroup extends GroupEvaluationMeasure {
    ...
    public GroupEvaluationMeasureResult getMeasureResult(
        ...
        MeanIterative meanMAEbyGroups = new MeanIterative();

        maeGroups.values().stream()
                .filter(maeOfGroup -> !maeOfGroup.isEmpty())
                .forEach(maeOfGroup -> meanMAEbyGroups.addValue(maeOfGroup.getMean()));

        if (meanMAEbyGroups.isEmpty()) {
            return new GroupEvaluationMeasureResult(this, Double.NaN);
        } else {
            double mae = meanMAEbyGroups.getMean();
