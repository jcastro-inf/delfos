...
public final class KnnMemoryBasedNWR_itemWeighted_TaskExecutor implements SingleTaskExecute<KnnMemoryBasedNWR_itemWeighted_Task> {

    public KnnMemoryBasedNWR_itemWeighted_TaskExecutor() {
        super();
    }

    @Override
    public void executeSingleTask(KnnMemoryBasedNWR_itemWeighted_Task task) {
        int idUser = task.idUser;
        int idNeighbor = task.idNeighbor;
        KnnMemoryBasedNWR_itemWeighted rs = task.rs;
        RatingsDataset<? extends Rating> ratingsDataset = task.ratingsDataset;
        Map<Integer, Double> itemWeights = task.itemWeights;

        if (idUser == idNeighbor) {
            return;
        }
        CollaborativeSimilarityMeasure similarityMeasure_ = (CollaborativeSimilarityMeasure) rs.getParameterValue(KnnMemoryBasedNWR_itemWeighted.SIMILARITY_MEASURE);

        Byte defaultRatingValue_ = 0;
        boolean defaultRating_ = (Boolean) rs.getParameterValue(KnnMemoryBasedNWR_itemWeighted.DEFAULT_RATING);
        if (defaultRating_) {
            defaultRatingValue_ = ((Integer) rs.getParameterValue(KnnMemoryBasedNWR_itemWeighted.DEFAULT_RATING_VALUE)).byteValue();
        }

        boolean inverseFrequency_ = (Boolean) rs.getParameterValue(KnnMemoryBasedNWR_itemWeighted.INVERSE_FREQUENCY);
        float caseAmp = ((Number) rs.getParameterValue(KnnMemoryBasedNWR_itemWeighted.CASE_AMPLIFICATION)).floatValue();
        boolean relevanceFactor_ = (Boolean) rs.getParameterValue(KnnMemoryBasedNWR_itemWeighted.RELEVANCE_FACTOR);
        int relevanceFactorValue_ = (Integer) rs.getParameterValue(KnnMemoryBasedNWR_itemWeighted.RELEVANCE_FACTOR_VALUE);
        ...
        if (!defaultRating_) {
            ...
            for (int idItem : intersection) {
                ...
                float weight = itemWeights.get(idItem).floatValue();
                common.add(new CommonRating(RecommendationEntity.ITEM, idItem, RecommendationEntity.USER, idUser, idNeighbor, d1, d2, weight));
            }
        } else {
